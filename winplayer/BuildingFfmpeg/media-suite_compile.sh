...
if [[ $x264 != no ]]; then
    ...
    if do_vcs "https://code.videolan.org/videolan/x264.git" ||
        ...
        if [[ $standalone = y && $x264 =~ (full|fullv) ]]; then
            ...
            do_vcs "https://git.ffmpeg.org/ffmpeg.git#tag=release/3.3"
...
if [[ $ffmpeg != no ]]; then
    ...
    if do_vcs "https://git.ffmpeg.org/ffmpeg.git#tag=release/3.3"; then
...
if [[ $cyanrip = y ]]; then
    ...
    if do_vcs "https://github.com/cyanreg/cyanrip.git"; then
        ...
        if flavor=cyan do_vcs "https://git.ffmpeg.org/ffmpeg.git#tag=release/3.3"; then
